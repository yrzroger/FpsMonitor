...
RefreshRateOverlay::RefreshRateOverlay(ftl::Flags<Features> features)
      : mFeatures(features), mSurfaceControl(createSurfaceControlHolder()) {
    if (!mSurfaceControl) {
...
auto RefreshRateOverlay::getOrCreateBuffers(float  displayFps, float  renderFps) -> const Buffers& {
    ...
    BufferCache::const_iterator it =
            mBufferCache.find({static_cast<int>(std::round(displayFps)), static_cast<int>(std::round(renderFps)), transformHint});
    if (it == mBufferCache.end()) {
        ...
        const int displayIntFps = std::clamp(static_cast<int>(std::round(displayFps)), minFps, maxFps);
        const int renderIntFps = static_cast<int>(std::round(renderFps));
...
void RefreshRateOverlay::changeRefreshRate(float  displayFps, float  renderFps) {
